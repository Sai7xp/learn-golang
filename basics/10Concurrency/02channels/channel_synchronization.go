...
func PrintEvenOddInSync() {
	evenCh, oddCh := make(chan bool), make(chan bool) // Channel with buffer size 1
...
func printEvenNumbers(evenCh chan bool, oddCh chan bool, wg *sync.WaitGroup) {
	defer wg.Done()

	for i := 2; i <= 10; i += 2 {
		<-evenCh // Wait for signal from odd numbers
...
func printOddNumbers(evenCh chan bool, oddCh chan bool, wg *sync.WaitGroup) {
	defer wg.Done()

	for i := 1; i < 10; i += 2 {
		<-oddCh // Wait for signal from even numbers

		fmt.Println(i, "is odd")
		// time.Sleep(time.Millisecond * 500)

		evenCh <- true // Signal back to even numbers
	}
